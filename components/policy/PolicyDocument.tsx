...
export function PolicyDocument({
  ...
  return (
    <section className="min-h-screen bg-surface-1 py-8 lg:py-12">
      <div className="mx-auto w-full max-w-[1280px] px-4 sm:px-6 lg:px-8">
        <div className="grid gap-8 lg:grid-cols-12">
          <aside className="hidden lg:col-span-3 lg:block">
            <div className="sticky top-24 rounded-2xl border border-border-subtle bg-surface-3 p-6 shadow-[0_14px_30px_rgba(0,0,0,0.25)]">
              ...
              <nav className="space-y-1">
                {content.sections.map((section) => (
                  <a
                    key={section.id}
                    href={`#${section.id}`}
                    className="block rounded-lg px-3 py-2.5 text-sm text-muted transition-colors hover:bg-white/10 hover:text-gray-100"
                  >
                    {section.title}
                  </a>
                ))}
          ...
          <main className="lg:col-span-9">
            <div className="mb-8 rounded-3xl border border-border-subtle bg-gradient-to-br from-secondary/20 via-secondary/10 to-surface-4 p-8 lg:p-12">
              <h1 className="text-3xl font-bold text-gray-100 lg:text-4xl">{content.title}</h1>
              <p className="mt-6 text-lg leading-relaxed text-muted-soft">{content.intro}</p>
            </div>

            <div className="mb-8 lg:hidden">
              <details className="overflow-hidden rounded-2xl border border-border-subtle bg-surface-3 shadow-[0_14px_30px_rgba(0,0,0,0.25)]">
            ...
            <div className="space-y-6">
              {content.sections.map((section) => (
                <section
                  key={section.id}
                  id={section.id}
                  className="scroll-mt-24 rounded-2xl border border-border-subtle bg-surface-4 p-6 shadow-[0_14px_30px_rgba(0,0,0,0.2)] lg:p-8"
                >
                  <h2 className="text-xl font-bold text-gray-100 lg:text-2xl">{section.title}</h2>
                  {section.description && <p className="mt-4 leading-relaxed text-muted-soft">{section.description}</p>}
                  ...
                  {section.note && (
                    <div className="mt-4 rounded-xl border-l-4 border-secondary bg-secondary/15 p-4">
                      <p className="text-sm italic leading-relaxed text-muted-success">{section.note}</p>
                    </div>
                  )}
                </section>
              ))}
